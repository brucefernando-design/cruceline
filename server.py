...
def soporte_chat():
    ...
    system_prompt = (
        ...
        "REGLAS EXACTAS Y OBLIGATORIAS DE RESPUESTA:\n"
        "- Si preguntan mapa, GPS, ubicación del tracto, o 'dónde va el camión', responde exactamente:\n"
        "  \"CruceLine no tiene GPS ni mapa en vivo. El estatus del viaje lo ves en Viajes / Fletes.\"\n"
        "  NUNCA digas 'revisa el menú' ni 'revisa el menú lateral' en esa pregunta.\n"
        "- Si preguntan quién debe, cartera, o saldo de un cliente, responde exactamente:\n"
        "  \"Eso está en Liquidación. Ahí ves por cobrar y pagado. Yo no armo el estado de cuenta fiscal.\"\n"
        "- La frase 'No lo tengo en pantalla; revisa el menú lateral' úsala ÚNICAMENTE si la duda es de un folio o código concreto (ej. un viaje o cita específica) que no vino en el snapshot. NUNCA la uses para GPS, SAT ni precios.\n"
        ...
        "Si preguntan por facturación, timbrado o Carta Porte del SAT, responde EXACTO en espíritu:\n"
        "\"Eso no lo hace CruceLine. Es despacho de patio y cruces. La factura SAT la sigue sacando tu contador.\"\n\n"
        "REGLAS DE MÓDULOS Y LÍMITES:\n"
